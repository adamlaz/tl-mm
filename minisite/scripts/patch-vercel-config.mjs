/**
 * Injects a standalone Vercel Edge Middleware for HTTP Basic Auth into the
 * Build Output. The @astrojs/vercel adapter doesn't support middleware for
 * prerendered pages, so we create the Edge Function directly.
 *
 * - Creates functions/_middleware.func/ with auth logic
 * - Patches config.json to run middleware before the filesystem handler
 */
import { readFileSync, writeFileSync, mkdirSync } from 'node:fs';

const MIDDLEWARE_DIR = '.vercel/output/functions/_middleware.func';
const CONFIG_PATH = '.vercel/output/config.json';

// ── 1. Write the Edge Middleware function ──
...
<head>
  ...
  <style>
    ...
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    h1 { font-family: 'Noto Sans Display', 'Noto Sans', system-ui, sans-serif;
...
writeFileSync(`${MIDDLEWARE_DIR}/index.js`, middlewareCode.trim() + '\n');
writeFileSync(`${MIDDLEWARE_DIR}/.vc-config.json`, JSON.stringify({ runtime: 'edge', entrypoint: 'index.js' }, null, '\t') + '\n');
console.log('[patch] Created standalone Edge Middleware for auth');

// ── 2. Patch config.json to run middleware before filesystem ──

const config = JSON.parse(readFileSync(CONFIG_PATH, 'utf-8'));
const hasMiddlewareRoute = config.routes?.some((r) => r.middlewarePath === '_middleware');

if (!hasMiddlewareRoute) {
  config.routes.unshift({ src: '/.*', middlewarePath: '_middleware', continue: true });
  writeFileSync(CONFIG_PATH, JSON.stringify(config, null, '\t') + '\n');
  console.log('[patch] Injected middleware route before filesystem handler');
}
